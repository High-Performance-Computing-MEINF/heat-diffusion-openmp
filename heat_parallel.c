...
void solve_heat(double *grid, double *new_grid, int local_nx, int ny, double r,
                int rank, int size) {
  MPI_Status status;
  // Exchange boundaries
  if (rank > 0) {
    MPI_Sendrecv(&grid[1 * ny], ny, MPI_DOUBLE, rank - 1, 0, &grid[0 * ny], ny,
...
int main(int argc, char *argv[]) {
  ...
  MPI_Scatterv(global_grid, counts, displs, MPI_DOUBLE, &grid[1 * ny],
  ...
  if (rank == 0)
    printf("Hybrid Execution Time (MPI+OpenMP): %.6f seconds\n", t1 - t0);

  MPI_Gatherv(&grid[1 * ny], local_nx * ny, MPI_DOUBLE, global_grid, counts,
              displs, MPI_DOUBLE, 0, MPI_COMM_WORLD);

  if (rank == 0) {
    char filename[256];
    snprintf(filename, sizeof(filename), "output_hybrid_%d_%d_mpi%d_omp%d.bmp",
             nx, steps, size, num_threads);
